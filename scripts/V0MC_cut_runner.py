...
Method:
  ...
  Step 2: Run the run_step0_MC.sh script to produce the output files
...
if __name__ == "__main__":
  ...
  for i in range(len(cut_parameters)):
    set_cut_value('data/V0MC/json_strangenesstutorialMC_sig.json', i, default_cut_values[i])
  ...
  for cut_value in cut_values:
    ...
    os.chdir("data/V0MC")
    os.system("./run_step0_MC.sh")
    os.chdir("../../")

    # Rename the output file to avoid overwriting
    os.system("mv data/V0MC/AnalysisResults.root output/V0MC/AnalysisResults_{}_{}.root".format(cut_parameters[par_index], cut_value))
