...
////////// anti-lambda efficiency //////////////

TH1F* EfficiencyCorrection::effAntiLambdaIndv(std::string MCFile,
                                              std::vector<double>& bins,
                                              bool draw) {
  gROOT->Reset();
  ...
  // save generated pt histogram as hpt
  f->cd("strangeness_tutorial/genParticles");
  TH1F* hpt = (TH1F*)gDirectory->Get("hPtAntiLambdaGen");
  if (hpt == nullptr) {
    std::cout << "hpt was not read" << std::endl;
    exit(1);
  }
  ...
  // save true reconstructed pt histogram as hptrue
  f->cd("strangeness_tutorial/kLambda");
  TH1F* hptrue = (TH1F*)gDirectory->Get("hPtAntiLambdaIndvTrueRec");
  if (hptrue == nullptr) {
    std::cout << "hptrue was not read" << std::endl;
    exit(1);
  }
  ...
  if (draw) {
    ...
    // dont draw the legend
    heff->SetStats(0);
    c1->SaveAs("efficiency_AntiLambda.pdf");
    delete c1;
  }
  return heff;
}

TH1F* EfficiencyCorrection::EfficiencyCorrectionAntiLambdaIndv(
    std::string DataFile, std::string MCFile, std::vector<double>& bins,
    double ptCut, bool draw) {
  TFile* f = new TFile(DataFile.c_str());
  f->cd("strangeness_tutorial/kLambda");
  TH1F* hpt = (TH1F*)gDirectory->Get("hPtAntiLambdaIndvSelected");
  if (hpt == nullptr) {
    std::cout << "hpt was not read" << std::endl;
    exit(1);
  }
  ...
  TH1F* heff = effAntiLambdaIndv(MCFile, binsFiltered, draw);
  TH1F* hpt_rebin = new TH1F("hpt_rebin", "hpt_rebin", binsFiltered.size() - 1,
                             &binsFiltered[0]);
  ...
  if (draw) {
    ...
    // dont draw the legend
    hpt_corrected->SetStats(0);
    c2->SaveAs("efficiency_corrected_AntiLambda.pdf");
    delete c2;
  }
  return hpt_corrected;
}

///////// lambda indv efficiency //////////////

TH1F* EfficiencyCorrection::effLambdaIndv(std::string MCFile,
                                          std::vector<double>& bins,
                                          bool draw) {
  gROOT->Reset();
  ...
  // save true reconstructed pt histogram as hptrue
  f->cd("strangeness_tutorial/kLambda");
  TH1F* hptrue = (TH1F*)gDirectory->Get("hPtLambdaIndvTrueRec");
  if (hptrue == nullptr) {
    std::cout << "hptrue was not read" << std::endl;
    exit(1);
  }
  ...
  if (draw) {
    ...
    // dont draw the legend
    heff->SetStats(0);
    c1->SaveAs("efficiency_LambdaIndv.pdf");
    delete c1;
  }
  return heff;
}

TH1F* EfficiencyCorrection::EfficiencyCorrectionLambdaIndv(
    std::string DataFile, std::string MCFile, std::vector<double>& bins,
    double ptCut, bool draw) {
  TFile* f = new TFile(DataFile.c_str());
  f->cd("strangeness_tutorial/kLambda");
  TH1F* hpt = (TH1F*)gDirectory->Get("hPtLambdaIndvSelected");
  if (hpt == nullptr) {
    std::cout << "hpt was not read" << std::endl;
    exit(1);
  }
  ...
  TH1F* heff = effLambdaIndv(MCFile, binsFiltered, draw);
  TH1F* hpt_rebin = new TH1F("hpt_rebin", "hpt_rebin", binsFiltered.size() - 1,
                             &binsFiltered[0]);
  ...
  if (draw) {
    ...
    // dont draw the legend
    hpt_corrected->SetStats(0);
    c2->SaveAs("efficiency_corrected_LambdaIndv.pdf");
    delete c2;
  }
  return hpt_corrected;
}